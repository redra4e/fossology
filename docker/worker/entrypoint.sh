...
# Sets up SSH authorized_keys from a mounted secret and starts sshd.
...
if [ -f /run/secrets/ssh/authorized_keys ]; then
    ...
    echo "[worker] SSH authorized_keys installed for fossy."
else
...
if [ -f /usr/local/etc/fossology/Db.conf ]; then
    ...
    echo "[worker] Agents will not be able to connect to the database."
fi

# Regenerate host keys if missing (fresh emptyDir or stripped image)
